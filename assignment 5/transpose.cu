...
{
	//tiling	
	int i =  threadIdx.x + K * blockIdx.x;
	int j = threadIdx.y + K * blockIdx.y;

	//set out value to final transposed matrix
	out[j + i*N] = in[i + j*N];
}
...
transpose_parallel_per_element_tiled_shared(float in[], float out[])
{
	...
	//set x and y values for threads
	int x = threadIdx.x; 
	int y = threadIdx.y;
	...
	//add transposed matrix to shared memory
	tile[y][x] = in[(in_i + x) + (in_j + y)*N];

	//wait for all threads in block to finish before continuing
	__syncthreads();

	//use shared memory to set out value to final transposed matrix
